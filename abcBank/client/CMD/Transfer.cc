...
void Transfer::execute(BankSession& session)
{
  ...
  // 转账请求包 包体
  // account_id 6bytes + pass 16 bytes + money + other_account_id 6bytes
  // pack account_id
  ...
  // pack other_account_id
	auto other_account_id = session.getRequestItem("OTHER_ACCOUNT_ID");
	packStream.writeBytesFrom(other_account_id.data(), ACCOUNT_ID_LENGTH);
