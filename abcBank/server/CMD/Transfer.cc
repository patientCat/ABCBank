...
void Transfer::execute(BankSession& session)
{
  ...
  unpack >> money;
  // 解包other_account_id
  char other_account_id[ACCOUNT_ID_LENGTH+1] = {0};
  unpack.readBytesTo(other_account_id, ACCOUNT_ID_LENGTH);
  ...
  BankService service;
  Account account;
  account.account_id = stoi(string(account_id));
  account.pass = pass;
  account.money = stod(money);
  LOG_INFO << account.account_id;
  LOG_INFO << account.pass;
  LOG_INFO << account.money;
  LOG_INFO << stoi(other_account_id);

  int ret = service.transfer(account, other_account_id);
  if(ret == 0)
  {
    error_code = 0;
    LOG_INFO << "[" 
  ...
  {
    error_code = 5;
    strncpy(error_msg, "peer account doesn't exist", sizeof(error_msg));
    LOG_INFO << other_account_id << " "<< error_msg;
  }
